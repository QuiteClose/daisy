...
# symlink — so multiple drafts can coexist while deciding which (if any) to
# promote. Promote one later with `plan-pickup.sh`.
# Usage: plan-new.sh [--spec] <description> [project-tag]
...
if [ -z "$DESCRIPTION" ]; then
    ...
    echo "Usage: plan-new.sh [--spec] <description> [project-tag]" >&2
    exit 1
fi
...
if [ "$SPEC_MODE" = true ]; then
    PLAN_FILE="$DAISY_WORKSPACE/${SLUG}_PLAN.md"
    FILENAME="${SLUG}_PLAN.md"
    if [ -e "$PLAN_FILE" ]; then
        echo "Error: ${FILENAME} already exists in this workspace" >&2
        exit 1
    fi
else
...
if [ "$SPEC_MODE" = true ]; then
    # Add *_PLAN.md to .gitignore if not already present
    ...
    echo "   Not registered with Daisy — promote later with: daisy plan-pickup $PLAN_FILE"
else
