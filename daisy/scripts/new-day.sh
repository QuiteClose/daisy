#!/usr/bin/env bash
# Invocation: run as `daisy new-day` — do not execute this file directly.
...
if [ "$1" = "--healthcheck" ]; then
    ...
    if [ ! -f "$DAISY_ROOT/daisy/templates/journal-day.md" ]; then
...
TEMPLATE=$(cat "$DAISY_ROOT/daisy/templates/journal-day.md")
...
# Commit changes
daisy commit --home "New day: $DATE $DAY"
