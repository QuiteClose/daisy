...
short_description: Plan huge work as trackable investigation tickets
...
- **Grilling** (HITL): Conversation via the /quizzing and /domain-modeling skills, one question at a time. The default case.
...
1. **Name the destination.** Run a `/quizzing` and `/domain-modeling` session to pin down what this map is finding its way to — the spec, decision, or change. The destination fixes the scope, so it's settled first.
...
3. Resolve it — **zoom as needed**: fetch the full body of any related or closed ticket on demand; invoke the skills the `## Notes` block names. If in doubt, use `/quizzing` and `/domain-modeling`.
